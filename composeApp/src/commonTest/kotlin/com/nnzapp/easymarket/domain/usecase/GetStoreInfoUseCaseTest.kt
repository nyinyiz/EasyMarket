...
class GetStoreInfoUseCaseTest {
    ...
    private val mockStoreRepository = object : StoreRepository {
        var resultToReturn: Result<Store> = Result.success(
    ...
    @Test
    fun `invoke delegates to store repository`() = runBlocking {
        // Given
        val testStore = Store(
            name = "Amazing Store",
            rating = 3.8,
    ...
    @Test
    fun `invoke handles store with edge case values`() = runBlocking {
        // Given
        val edgeCaseStore = Store(
            name = "",
            rating = 0.0,
            openingTime = "24:00",
            closingTime = "00:00"
